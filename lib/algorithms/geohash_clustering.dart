...
class GeohashClustering<T extends ClusterItem> extends ClusterAlgorithm<T> {
  ...
  List<Cluster<T>> _computeClusters(
    ...
    final String nextGeohash = inputItems[0].geohash.substring(0, precision);
    final List<T> items = inputItems
        .where((p) => p.geohash.substring(0, precision) == nextGeohash)
        .toList();

    clusters.add(Cluster<T>.fromItems(items));

    final List<T> newInputList = List.from(
      inputItems.where((i) => i.geohash.substring(0, precision) != nextGeohash),
    );
